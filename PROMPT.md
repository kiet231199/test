...
- `native_runtime.py` fixes native decoder, filter, and exceptional-copy worker
  limits at one thread.
...
- Encoded input and reference decoders use one native thread for every metric.
  Every PSNR filter graph, including raw-only PSNR, also uses one thread.
